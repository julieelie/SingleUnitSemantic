function [filename]=FirsVoc_extract(SoundPath)
...
Data=dir(fullfile(SoundPath,'*.wav'));
...
Sound = cell(nfiles,1);
Stim_names = cell(nfiles,1);
histDur=[];

%Loop through files, detect sound periods
for isound = 1:nfiles
    fprintf('detect sound %d/%d\n', isound, nfiles)
    Stim_names{isound} = Data(isound).name;
    [sound_in, samprate] = wavread(fullfile(SoundPath,Stim_names{isound}));
...
for isound = 1:nfiles
    ...
    Spectroto{n1section}= to;
    Spectrofo{n1section}= fo;
    ...
    Original_wavfiles{n1section}=Stim_names(isound);
    Eidentity{n1section}=Stim_names{isound}(1:10);
...
Res.Original_wavfiles=Original_wavfiles(1:n1section); % This is the original name of the wav file in the vocalization bank.
Res.WavIndices = WavIndices(1:n1section); % begining and end indices of each section within the Original_wavfiles, just to be able to retrieve the wavform if needed
Res.SectionWave=SectionWave(1:n1section);
Res.SectionLength = SectionLength(1:n1section); % duration of each section in ms
Res.Eidentity=Eidentity(1:n1section); % Name tag of the emitter
Res.Spectro=Spectro(1:n1section);
Res.Spectroto=Spectroto(1:n1section);
Res.Spectrofo=Spectrofo(1:n1section);
Res.VocDuration=dur;
...
if ismac()
        ...
        if strcmp(strtok(username), 'frederictheunissen')
            ...
            elseif strncmp('/auto/fdata/julie',stim_name, 17)
                filename = fullfile('/Users','frederictheunissen','Documents','Data','Julie','FirstVoc_extract_NeuroBank.mat');
            end
        elseif strcmp(strtok(username), 'elie')
            filename = fullfile('/Users','elie','Documents','MATLAB','data','matfile','FirstVoc_extract_NeuroBank.mat');
        end
else
    filename=fullfile('/auto','k6','julie','matfile','FirstVoc_extract_NeuroBank.mat');
end
