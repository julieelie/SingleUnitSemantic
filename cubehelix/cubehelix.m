...
% Colormaps of Dave Green's Cubehelix colorscheme, with brightness control.
%
...
% The colorscheme is made of nodes along a tapered helix in the RGB color cube,
...
%  lo  = LogicalMatrix, same size as <map>, true where scheme values<0 were clipped.
%  hi  = LogicalMatrix, same size as <map>, true where scheme values>1 were clipped.
%
...
if nargin<=5
    ...
    assert(numel(rng)==2&&isreal(rng),'Input <rng> must be a 1by2 real numeric vector.')
end
%
% ### Core Function ###
%
...
for n = 1:N
    ...
    fra = ((n-1)/(N-1))^gamma;
    amp = hue * fra * (1-fra) / 2;
    fra = rng(1)*(1-fra) + rng(2)*fra; % Brightness range control.
