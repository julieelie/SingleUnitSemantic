...
% Rediscover the parameter values for Cubehelix colorscheme colormaps.
%
% (c) 2013 Stephen Cobeldick
%
% So you have a nice Cubehelix colormap, and you can't remember the exact
...
% Outputs:
%  vec = NumericVector, [start,rots,hue,gamma], Cubehelix parameter values.
%  rng = NumericVector, [lb,ub], brightness levels of the colormap's endnodes. Size 1by2.
%
% [vec,rng] = cubehelix_view(map)

assert(ismatrix(map)&&size(map,2)==3&&all(0<=map(:)&map(:)<=1),'Input argument <map> must be a colormap.')
%
% Parameter bounds:
lb = [0,-20,0,0];
ub = [3,+20,3,3];
...
F = @(p)map-cubehelix(N,p(1),p(2),p(3),p(4),rng);
