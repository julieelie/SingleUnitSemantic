...
load meanSnipsAllSolveig  % This is from Solveig's data set
tdt25k = 24414.0625;
...
[indKmeans,clusterMeans,sumD,distanceToMean] = kmeans(coords(:,1:3),5);

% Load the annotated snips - warning - this will overide allSnips
% load meanSnipsAnn
load meanSnipsAllSolveigAnn
...
ind_d = find(typeSnips == 'd');
plot(coords(ind_l,1), coords(ind_l, 2), '+k');
legend('narrow', 'wide', 'large', 'double');
...
plot(coords(indKmeans==5,1), coords(indKmeans==5, 2), '+y');
plot(clusterMeans(1,1),clusterMeans(1,2),'x',...
 ...
 plot(clusterMeans(3,1),clusterMeans(3,2),'gx',...
  ...
  plot(clusterMeans(4,1),clusterMeans(4,2),'kx',...
     'MarkerSize',16,'LineWidth',4)
   plot(clusterMeans(5,1),clusterMeans(5,2),'yx',...
     'MarkerSize',16,'LineWidth',4)
...
plot3(coords(indKmeans==5,1), coords(indKmeans==5,2), coords(indKmeans==5,3), '+y');
hold off;
...
figure(5);
nPlot = 30;
titleName = {'G1'; 'G2'; 'G3'; 'G4'; 'G5'};
        

for nk=1:5  % for the four clusters
    subplot(1,5,nk);
...
for i=1:nSingle
    ...
    % Assign Cluster type
    if indKmeans(i) == 3 || indKmeans(i) == 5
        spikeType{i} = 'narrow';
    elseif indKmeans(i) == 1 || indKmeans(i) == 2
        spikeType{i} = 'wide';
    elseif indKmeans(i) == 4
...
[meanSpikeWidth, semSpikeWidth, stdSpikeWidth, nameSpikeWidth] = grpstats(spikeWidth, spikeType, {'mean'; 'sem'; 'std'; 'gname'});
[p, table, stats] = anova1(spikeWidth, indKmeans);
fprintf(1,'Mean Spike Widths at half maximum for each group:\n');
for ig=1:length(meanSpikeWidth)
    
    fprintf(1,'\t%s %.2f +- %.3f sd: %.3f\n', nameSpikeWidth{ig}, meanSpikeWidth(ig), semSpikeWidth(ig), stdSpikeWidth(ig));
end

% save spikeShapeResultsSolveig allMeanSnips snrMinMaxUnits indL5 indG5 allNames allSortType spikeType spikeWidth
